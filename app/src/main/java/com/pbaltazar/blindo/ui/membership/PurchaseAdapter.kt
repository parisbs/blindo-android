...
import com.pbaltazar.blindo.databinding.ItemInappPurchaseBinding
import com.pbaltazar.blindo.databinding.ItemSubscriptionPurchaseBinding
import com.pbaltazar.blindo.entities.Purchase
import com.pbaltazar.blindo.entities.purchases.enums.ProductType

class PurchaseAdapter : Adapter<Purchase>() {

    override fun bind(item: Purchase, viewHolder: ViewHolder) {
        when (viewHolder) {
            is InAppPurchaseViewHolder -> {}
            is SubscriptionPurchaseViewHolder -> viewHolder.itemBinding.apply {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder = when (viewType) {
        R.layout.item_inapp_purchase -> InAppPurchaseViewHolder(
            ItemInappPurchaseBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
        R.layout.item_subscription_purchase -> SubscriptionPurchaseViewHolder(
            ItemSubscriptionPurchaseBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
        else -> super.onCreateViewHolder(parent, viewType)
    }

    override fun getItemViewType(position: Int): Int = when (items[position].kind) {
        ProductType.INAPP -> R.layout.item_inapp_purchase
        ProductType.SUBSCRIPTION -> R.layout.item_subscription_purchase
    }
}
